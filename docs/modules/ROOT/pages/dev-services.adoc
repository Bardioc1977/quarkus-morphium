...
By default, Dev Services starts MongoDB as a single-node replica set — matching the default
behavior of MongoDB 8+. This enables multi-document transactions, change streams, and
`@MorphiumTransactional` out of the box.
...
| Driver
| The active Morphium driver implementation (e.g. `PooledDriver`, `InMemoryDriver`).

| Status
| `Connected` when the Morphium instance is available; error message otherwise.
|===
