...
| `true`
| Automatically create / verify indexes on write operations.
...
| –
| Path to the keystore file (JKS or PKCS12) containing the client certificate for X.509 / mutual TLS.
...
| `true`
| Enable Morphium health checks (liveness, readiness, startup). Requires `quarkus-smallrye-health` on the classpath.
|===
