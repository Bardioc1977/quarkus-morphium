...
multi-document atomicity, so transaction tests must use Dev Services with replica-set mode:
...
| best-effort
| No true atomicity — use Dev Services with `replica-set=true`

| Change streams
| no
| Use Dev Services

| X.509 / SCRAM auth commands
| no
| Logged as WARN, safely ignored
|===
